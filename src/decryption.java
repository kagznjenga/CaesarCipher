...
/*In order to do this I will be using the Caesar Cipher method to shift the letters
 * in the user's input by the specified number of times that they type in as their key
 * This page contains the code for decrypting text
 * Execute the encrypting code first and then copy the result as your input into this program*/
...
public class decryption {
  ...
  public static String decrypt(String plainText, int shift) {
    ...
    for(int i = 0; i < length; i++) {
      ...
      /* The if statement below checks if the character at each index is a letter
       * If it is not a letter the program doesn't change it and goes to the next index */
      ...
      if(Character.isLetter(character)) {
        ...
        if(Character.isLowerCase(character)) {
          ...
          /* The if/else statement below checks if the character shift is out of bounds in the lower case alphabet.
           * If it is out of bounds it goes to the beginning of the alphabet until the last count is done*/
          if(shiftedChar < 'a') {
            cipherText += (char)(character + (26 - shift));
          }
          else {
            //The code below saves the new character that has been shifted since it is within bounds.
